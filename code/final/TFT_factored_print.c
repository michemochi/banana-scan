...
static PT_THREAD (protothread_print(struct pt *pt))
{
    ...
    back_color = ILI9340_BLUE;
     tft_setTextColor(text_color);  
     sprintf(buffer, "PRINT TEST         20         30   ");
     printLine(0, buffer, text_color, back_color);
      ...
      while(1) {
        ...
        if (line_count==7) back_color = ILI9340_YELLOW;
        if (line_count==13) back_color = ILI9340_BLUE;
        if (line_count==19) back_color = ILI9340_GREEN;
        if (line_count==25) back_color = ILI9340_RED;
        tft_setTextColor(text_color); 
        
        sprintf(buffer, "Line Number %d Time %d", line_count, PT_GET_TIME());
        printLine(line_count, buffer,  text_color, back_color);
