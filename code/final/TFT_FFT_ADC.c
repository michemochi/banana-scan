...
#define nPixels 256
fix14 v_in[nSamp] ;
 #define sample_rate 500000 // 500000 works
...
{
    PT_BEGIN(pt);
    static int sample_number, max_fr_index ;
    static fix14 zero_point_4 = float2fix14(0.4) ;
    static fix14 frr, fii, max_fr ;
    // approx log calc ;
    ...
    while(1) {
        ...
        // start at 1 to omit DC
        for (sample_number = 1; sample_number < nPixels; sample_number++) {  
            ...
            // find the max
            // start at index 3 to avoid DC spread
            if (frr>max_fr && sample_number>3) {
                max_fr = frr ;
            ...
            if (sample_number==max_fr_index) temp = frr;
            sx = frr;
            y=1; ly=0;
            while(sx>1) {
        ...
        sprintf(buffer, "FFT cpu cycles %d", (ReadTimer4())*8);
        printLine2(0, buffer, ILI9340_WHITE, ILI9340_BLACK);
       
        sprintf(buffer, "PEAK FREQ = %d      ", max_fr_index * sample_rate/(nSamp));
