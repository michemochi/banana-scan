...
// UART lock for two thread printing
int char_count ;
    ...
    PT_BEGIN(pt);
     ...
     mPORTASetPinsDigitalOut(BIT_0 );    //Set port as output
      while(1) {
        ...
        clr_right ;
        // num_char is maintained by the threads serial input function
        // PT_GetSerialBuffer
        // This could FAIL if PT_GetSerialBuffer leaves a number in  num_char upon exit
        // num_char should be zeroed by the function before exit, as it is in 2_2_2a
        cursor_pos(4,5+num_char);
        normal_text ;
